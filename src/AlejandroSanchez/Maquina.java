...
public class Maquina extends Jugador {
	 ...
	 * @see Colores
	 */
	public byte[] introducirCombSecreta(){
	byte combinacionSecreta[] = new byte[colores.length];

	switch(getTablero().getModo().getDificultad()) {
	
		case DIFICIL:
			for(int i=0; i<combinacionSecreta.length;i++) {
				combinacionSecreta[i] = (byte) (rnd.nextInt(combinacionSecreta.length) +1);
			}
		break;
	}
	 ...
	 * @see Colores
	 */
	public byte[] introducirCombinacion() {
		byte combinacion[] = new byte[super.getTablero().getCombinacion().getCeldas().length];
		int longitud = combinacion.length;
		
		if(!getTablero().isRepeticion()) {
			for(int i=0; i<longitud;i++) {
				combinacion[i] = (byte) (rnd.nextInt(combinacion.length) +1);
			}
		}
		else {
			for(int i=0; i<longitud;i++) {
